...
from utils.file import setup_directory, sort_function, parse_global_config, get_png_file_name
...
def main():
    ...
    if not os.path.exists(ANIMATION_DIRECTORY) or not os.path.exists(PFP_DIRECTORY):
        raise Exception("Missing animation directory or pfp directory, make sure to have generatePFP set to true")
    ...
    for file in os.listdir(PFP_DIRECTORY):
        ...
        animated_file_path = os.path.join(ANIMATION_DIRECTORY, f"{edition}.{output_type}")

        with open(os.path.join(PFP_DIRECTORY, file), 'rb') as binary_file:
            result_text = result_text.replace("image_here", encode_binary_file_to_string(binary_file))

        with open(animated_file_path, 'rb') as binary_file:
            result_text = result_text.replace("gif_here", encode_binary_file_to_string(binary_file))

        with open(os.path.join(OUTPUT_DIRECTORY, f"{edition}.html"), 'wt') as output_file:
